...
use futures::{StreamExt, future::ok, stream};
use bytes::Bytes;
...
async fn test_encode_endpoint() {
    ...
    assert!(resp.status().is_success(), "Response status is not success: {}", resp.status());
    
    // Parse response body
    let body = test::read_body(resp).await;
    let json_response: serde_json::Value = serde_json::from_slice(&body).expect("Failed to parse JSON response");
    
    // Check that the response contains the expected fields
    assert!(json_response.get("status").is_some(), "Missing status field");
    assert_eq!(json_response["status"], "success", "Status is not success");
    assert!(json_response.get("image_id").is_some(), "Missing image_id field");
    assert!(json_response.get("download_url").is_some(), "Missing download_url field");
}
...
async fn test_encode_no_image() {
    ...
    assert!(json_response["message"].as_str().unwrap().contains("Missing cover image"));
}
...
async fn test_encode_no_message() {
    ...
    assert_eq!(json_response["status"], "error");
    assert_eq!(json_response["error_code"], "validation_error");
    assert!(json_response["message"].as_str().unwrap().contains("Missing message"));
}
